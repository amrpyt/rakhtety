...
TASKS_DIR="D:/Vibe Coding/02-Real PRojects with Clints/رخصتي/.codex/tasks"
if [ -d "$TASKS_DIR" ]; then
  STALE_COUNT=$( (find "$TASKS_DIR" -maxdepth 1 -type d -mtime +1 2>/dev/null || true) | wc -l )
  if [ "$STALE_COUNT" -gt 0 ]; then
    echo "⚠️  Found $STALE_COUNT stale task directories in D:/Vibe Coding/02-Real PRojects with Clints/رخصتي/.codex/tasks/"
    echo "   These are leftover from crashed subagent sessions."
    echo "   Run: rm -rf D:/Vibe Coding/02-Real PRojects with Clints/رخصتي/.codex/tasks/*  (safe — only affects dead sessions)"
  fi
fi
```
