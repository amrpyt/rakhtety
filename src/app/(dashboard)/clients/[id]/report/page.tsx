...
export default function ClientReportPage() {
  ...
  useEffect(() => {
    async function loadReport() {
      ...
      try {
        const [clientRecord, workflowRecords] = await Promise.all([
          clientService.findById(clientId),
          workflowService.findByClientId(clientId),
        ])
        ...
        const workflowsWithSteps = await Promise.all(
          workflowRecords.map((workflow) => workflowService.getWithSteps(workflow.id))
        )
        const financialSummaries = await Promise.all(
          workflowsWithSteps.map((workflow) => financialService.calculateWorkflowSummary(workflow.id))
        )

        setClient(clientRecord)
        setWorkflows(workflowsWithSteps)
        setSummaries(
          Object.fromEntries(financialSummaries.map((summary) => [summary.workflow_id, summary]))
        )
      } catch (err) {
  ...
  return (
    <main className="report-page min-h-screen bg-white p-6 text-[#1f2933]">
      <div className="report-toolbar mx-auto mb-4 flex max-w-5xl items-center justify-between gap-3">
      ...
      <article className="mx-auto max-w-5xl rounded-[var(--radius-lg)] border border-[var(--color-border)] bg-white p-8 shadow-sm">
        <header className="mb-8 border-b border-[var(--color-border)] pb-5">
          ...
          <div className="mt-2 flex flex-wrap items-start justify-between gap-4">
            <div>
              <h1 className="text-3xl font-bold">تقرير ملف العميل</h1>
              <p className="mt-1 text-sm text-[var(--color-text-muted)]">
                تاريخ الإصدار: {dateTime(new Date().toISOString())}
              </p>
            </div>
            <div className="text-left text-sm text-[var(--color-text-muted)]">
        ...
        <section className="mb-8">
          ...
          <div className="grid grid-cols-2 gap-3 rounded-[var(--radius-md)] bg-[#f8fafc] p-4 text-sm">
                ...
                <div key={workflow.id} className="rounded-[var(--radius-md)] border border-[var(--color-border)] p-4">
                  ...
                  <ol className="space-y-2">
                    {workflow.steps.map((step) => (
                      <li key={step.id} className="grid grid-cols-[auto_1fr_auto] items-center gap-3 rounded-[var(--radius-md)] bg-[#f8fafc] p-3 text-sm">
